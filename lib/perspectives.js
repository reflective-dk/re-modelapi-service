...
Perspectives.prototype.employees = function(request, response, next) {
                ...
                .then(function(employments) {
                    return Promise.all([
                        ...
                        return forEachHierarchy(employments, units, hierarchies,
                                                employeePerspective, sort, toRows);
                    });
                });
        })
        .then(formatAndSend(response))
    ...
    function sort(employments, order) {
        var employmentsByUnit = {};
        employments.forEach(function(employment) {
            var unitId = _.get(employment, 'snapshot.employedAt.id');
            if (!unitId) {
                return;
            }
            var list = employmentsByUnit[unitId] = employmentsByUnit[unitId] || [];
            list.push(employment);
        });
        return [].concat.apply([], order.map(key => employmentsByUnit[key] || []));
    }
...
function unitPerspective(unit, hierarchy, upstream) {
    ...
    upstream[unit.id] = {
        Id: unit.id,
	EksterntId: Object.values(_.get(unit, 'snapshot.foreignIds', {})).join(', '),
 ...
 * order provided.
 */
function employeePerspective(employment, hierarchy, upstream) {
    ...
    var userAccounts = Object.values(_.get(employment, 'snapshot.userAccounts', {}))
        ...
        .join(', ');
    var stiFraRod = [ _.get(parent, 'StiFraRod'), _.get(parent, 'EnhedNavn') ]
        .filter(n => n).join(' < ') || undefined;
    var refs = upstream[unit.id] = upstream[unit.id] || [];
    refs.push({
        Id: employment.id,
	EksterntId: Object.values(_.get(employment, 'snapshot.foreignIds', {})).join(', '),
	Navn: _.get(employment, 'snapshot.name'),
        EnhedId: unit.id,
	EnhedEksterntId: Object.values(_.get(unit, 'snapshot.foreignIds', {})).join(', '),
	EnhedNavn: _.get(unit, 'snapshot.name'),
	StiFraRod: stiFraRod,
        StillingId: position.id,
	StillingEksterntId: Object.values(_.get(position, 'snapshot.foreignIds', {})).join(', '),
	...
	Telefon: Object.values(_.get(employment, 'snapshot.phoneNumbers', {})).join(', '),
	Email: Object.values(_.get(employment, 'snapshot.emailAddresses', {})).join(', '),
