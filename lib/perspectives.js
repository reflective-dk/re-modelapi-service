...
Perspectives.prototype.roleAssignments = function(request, response, next) {
                ...
                .then(function(assignments) {
                    return Promise.all([
                        ...
                        return forEachHierarchy(assignments, units, hierarchies,
                                                roleAssignmentPerspective, sort, toRows);
                    });
                });
        })
        .then(formatAndSend(response))
    ...
    function sort(assignments, order) {
        var assignmentsByUnit = {};
        assignments.forEach(function(assignment) {
            var unitId = _.get(assignment, 'snapshot.propagateFrom.id');
            if (!unitId) {
                return;
            }
            var list = assignmentsByUnit[unitId] = assignmentsByUnit[unitId] || [];
            list.push(assignment);
        });
        return [].concat.apply([], order.map(key => assignmentsByUnit[key] || []));
    }
 ...
 * order provided.
 */
function roleAssignmentPerspective(assignment, hierarchy, upstream) {
