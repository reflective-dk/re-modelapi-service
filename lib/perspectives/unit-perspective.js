...
module.exports = function(request, response, next) {
    ...
    return util.getContextFromHeader(request)
        .then(function(context) {
            return self.nowSnapshots(context, models.ro.classes.unit.id, activityFilter)
                .tap(units => self.expand(context, units, paths1))
                .tap(units => self.expand(context, units, paths2))
                .then(function(units) {
                    return self.nowSnapshots(context, models.entity.classes.hierarchy.id, activityFilter)
