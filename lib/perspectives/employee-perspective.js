...
module.exports = function(request, response, next) {
    ...
    return util.getContextFromHeader(request)
        .then(function(context) {
            var paths1 = [
              { path: 'snapshot.employedAt', class: perspectiveUtil.unitClassId(context) },
...
function employeePerspective(employment, hierarchy, upstream, pathFromRootMap,
    ...
    var ref = {
        ...
        EgetLederniveau: _.get(managerAssignment, 'snapshot.aliases.ledelsesniveau')
          || _.get(employment, 'snapshot.aliases.ledelsesniveau') || undefined,
    ...
    if (_.has(unit, 'snapshot.Institutionsnummer')) {
        ref['Institutionsnummer'] = _.get(unit, 'snapshot.Institutionsnummer');
    }
    if (_.has(unit, 'snapshot.foreignIds.institutionsnummer')) {
        ref['Institutionsnummer'] = _.get(unit, 'snapshot.foreignIds.institutionsnummer');
    }
