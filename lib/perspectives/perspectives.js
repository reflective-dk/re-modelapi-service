...
function Perspectives(api) {
  ...
  this.unitSnapshots = function(context, activationFilter) {
    return this.nowSnapshots(context, perspectiveUtil.unitClassId(context),
                             activationFilter);
  };
}
...
function pathToArgs(path, context, activationFilter) {
    var args = {
        context: context,
        expectMany: true,
        filterActive: activationFilter == 'active-only'
    };
    switch (true) {
