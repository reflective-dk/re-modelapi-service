...
module.exports = function(request, response, next) {
    ...
    var paths = [
        ...
        { path: 'snapshot.allocations',
          byIncomingRelation: { relatesTo: { class: models.ro.classes['system-access-allocation'].id },
                                relation: 'right' } }
    ];
    var activationFilter = _.get(request, 'body.activationFilter');
    return util.getContextFromHeader(request)
        .then(function(context) {
            return self.nowSnapshots(context, models.ro.classes.right.id, activationFilter)
                .tap(rights => self.expand(context, rights, paths, activationFilter))
